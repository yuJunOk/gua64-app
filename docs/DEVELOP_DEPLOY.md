...
| `npm run icons:android` | 根据 `public/logo.svg` 生成安卓各密度启动图标（修改 Logo 后执行） |
...
- **矢量（Android 8+ adaptive）**：`android/app/src/main/res/drawable/ic_launcher_foreground.xml` 与 `public/logo.svg` 图形一致；改 SVG 后请同步改该 XML（文件头注释已说明）。
- **各密度 PNG（旧版系统）**：修改 `public/logo.svg` 后执行 `npm run icons:android`，会重写 `mipmap-*dpi` 下的 `ic_launcher*.png`。
